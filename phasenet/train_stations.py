...
def read_args():
    ...
    parser.add_argument(
        "--model_dir", default=None, help="Checkpoint directory (default: None)"
    )
    ...
    parser.add_argument(
        "--num_plots", default=10, type=int, help="Plotting training results"
    )
    parser.add_argument(
        "--min_p_prob", default=0.3, type=float, help="Probability threshold for P pick"
    )
    parser.add_argument(
        "--min_s_prob", default=0.3, type=float, help="Probability threshold for S pick"
    )
    ...
    parser.add_argument(
        "--plot_figure", action="store_true", help="If plot figure for test"
    )
    parser.add_argument(
...
def train_fn(args, data_reader, data_reader_valid=None):
    ...
    with tf.compat.v1.name_scope("Input_Batch"):
        ...
        if data_reader_valid is not None:
            dataset_valid = data_reader_valid.dataset(
    ...
    with tf.compat.v1.Session(config=sess_config) as sess:
        tf.compat.v1.summary.FileWriter(log_dir, sess.graph)  # summary_writer
        ...
        for epoch in range(args.epochs):
            progressbar = tqdm(
                range(0, data_reader.num_data, args.batch_size),
            ...
            for _ in progressbar:
                ...
                progressbar.set_description(
                    "{}: epoch {}, loss={:.6f}, mean={:.6f}".format(
                        log_dir.split("/")[-1], epoch, loss_batch, train_loss.value
                    )
                )
            ...
            if data_reader_valid is not None:
                ...
                if valid_loss.value < best_valid_loss:
                    ...
                    saver.save(
                        sess, os.path.join(model_dir, "model_{}.ckpt".format(epoch))
                    )
            ...
            if args.plot_figure:
                pool.starmap(
                    ...
                    zip(
                        X_batch, preds_batch, [x.decode() for x in fname_batch], Y_batch
                    ),
                )
...
def test_fn(args, data_reader):
    ...
    with tf.compat.v1.Session(config=sess_config) as sess:
        ...
        test_loss = LMA()
        progressbar = tqdm(
            range(0, data_reader.num_data, args.batch_size), desc=args.mode
        )
        picks = []
        true_picks = []
        for _ in progressbar:
        ...
        calc_performance(picks, true_picks, tol=3.0, dt=data_reader.config.dt)  # metrics
